...
export default function ServiceDetailPage({ params }: { params: { id: string } }) {
  ...
  const packages = [
    { name: t('pkgBasic'),    desc: t('pkgBasicDesc'),    price: service.basePrice },
    { name: t('pkgStandard'), desc: t('pkgStandardDesc'), price: Math.round(service.basePrice * 1.6) },
    { name: t('pkgLuxury'),   desc: t('pkgLuxuryDesc'),   price: Math.round(service.basePrice * 2.4) },
  ];
    ...
    <div className="pb-32 md:pb-10">
      ...
      <div className="sticky top-14 z-40 bg-white dark:bg-[#2d2d30] border-b border-border-primary px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button className="p-1.5 text-text-muted hover:text-text-primary transition-colors" aria-label={t('bookmarkLabel')}>
            <Heart size={20} />
          </button>
          <button className="p-1.5 text-text-muted hover:text-text-primary transition-colors" aria-label={t('shareLabel')}>
            <Share2 size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="px-4 md:px-0 mt-4 space-y-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-2 text-sm">
            <div className="flex items-center gap-1">
              ...
              <span className="text-text-muted">({service.reviewCount} {t('reviewsUnit')})</span>
            </div>
        ...
        <div className="bg-white dark:bg-[#2d2d30] rounded-xl p-4 shadow-sm">
          <h2 className="font-semibold text-text-primary dark:text-white mb-2">{t('description')}</h2>
        ...
        {/* 套餐选择 */}
        <div className="bg-white dark:bg-[#2d2d30] rounded-xl p-4 shadow-sm">
          <h2 className="font-semibold text-text-primary dark:text-white mb-3">{t('selectPackage')}</h2>
        ...
        <div className="bg-white dark:bg-[#2d2d30] rounded-xl p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-text-primary dark:text-white">{t('reviews')}</h2>
            <button className="flex items-center gap-0.5 text-xs text-[#0d9488]">{t('viewAll')} <ChevronRight size={13} /></button>
          </div>
      ...
      <div className="fixed bottom-16 md:bottom-0 left-0 right-0 bg-white dark:bg-[#2d2d30] border-t border-border-primary px-4 py-3 flex items-center gap-3 z-40 md:static md:border-0 md:bg-transparent md:px-0 md:mt-4">
        <div>
          <p className="text-xs text-text-muted">{t('startingFrom')}</p>
